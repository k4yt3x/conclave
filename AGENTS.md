...
**Session management**: Session tokens use a sliding TTL (default 30 days). Every authenticated API request extends the token's expiry, so active clients stay logged in indefinitely. The `change_password` endpoint requires the current password for verification and invalidates all sessions. Clients auto-logout only on `AuthTokenExpired` error code (not all 401s) — auth header misconfiguration errors (wrong header name, missing Bearer prefix) are shown as errors without triggering logout.

**Error codes**: All `ErrorResponse` messages include a machine-readable `ErrorCode` enum alongside the human-readable message. Error codes use range-based numbering grouped by category: 0 = unspecified, 100–199 = input (`ERROR_CODE_INPUT_`), 200–299 = auth (`ERROR_CODE_AUTH_`), 300–399 = resource (`ERROR_CODE_RESOURCE_`), 400–499 = group operations (`ERROR_CODE_GROUP_`). Within each category, codes are ordered by when a user would encounter them (earliest/most common first). New codes MUST be added within the appropriate range and follow the category prefix. Clients MUST use `error_code` for control flow, not the message string. The server error enum carries the code in its `Unauthorized` variant (`Unauthorized { message, code }`); all other variants map 1:1 to a fixed code. Constructor helpers: `Error::token_expired()`, `Error::auth_misconfigured()`, `Error::not_member()`, `Error::not_admin()`.

**Sensitive data zeroization**: The `zeroize` crate is used to overwrite sensitive data in memory on drop. MLS signing keys use `Zeroizing<Vec<u8>>` in `MlsManager`. TUI password prompt fields use `Zeroizing<String>`. GUI password fields are explicitly zeroized via `Zeroize::zeroize()` after successful login or password change. `ApiClient::set_token()` zeroizes the old token before replacing it. `SessionState::save()` zeroizes the serialized TOML string after writing to disk. When adding new code that handles passwords, tokens, or key material: use `Zeroizing<T>` wrappers where the type flows through ownership, or call `Zeroize::zeroize()` explicitly before drop when wrappers aren't practical (e.g., GUI form fields bound to iced widgets).
...
**Public rooms**: Groups have a `GroupVisibility` enum (PRIVATE=1, PUBLIC=2) stored as an integer in the `groups.visibility` column (default PRIVATE). Admins toggle visibility with `/visibility <public|private>`. Public groups are discoverable via `GET /api/v1/groups/public` (returns `ListPublicGroupsResponse` with `PublicGroupInfo` entries) and discoverable in the TUI/GUI via `/discover [pattern]`. Non-members can join a public group via `POST /api/v1/groups/{group_id}/join`, which uses MLS external commit (RFC 9420 Section 12.4). The server adds the user as a member before the external commit is submitted. The `external_join` flow detects whether the joiner is a new member or an account-reset rejoin by checking whether the user has message history in the group. Private groups return `ERROR_CODE_GROUP_NOT_PUBLIC` (402) on the join endpoint. The `/join` command in TUI/GUI is extended to accept a group name for public room joining in addition to accepting invites.

**Bans**: Admins can `/ban <username>` to remove a user from a group and prevent them from rejoining. The ban performs an MLS removal commit (like `/kick`) plus adds the user to the `banned_users` table (keyed by `(group_id, user_id)`). Ban enforcement points: `join_public_group` handler, `accept_pending_invite` transaction, and `escrow_invite` handler all check `is_banned()` and return `ERROR_CODE_GROUP_BANNED` (403). Banning also cancels any pending invites for the target. `/unban <username>` removes the ban entry. `/banned` lists banned users (admin only). All three commands are in the Members category. The ban table has `ON DELETE CASCADE` from both `groups` and `users` tables.
